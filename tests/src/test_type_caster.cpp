...
namespace carma { namespace tests {
    ...
    arma::Cube<double> test_tc_out_cube(py::array_t<double> & arr) {
        arma::Cube<double> ones = arma::Cube<double>(
            arr.shape(0), arr.shape(1), arr.shape(2), arma::fill::ones
        );
...
void bind_test_tc_out_mat_rvalue(py::module &m) {
    m.def(
        "tc_out_mat_rvalue",
        &carma::tests::test_tc_out_mat_rvalue,
        "Test type caster"
    );
}
...
void bind_test_tc_out_row_rvalue(py::module &m) {
    m.def(
        "tc_out_row_rvalue",
        &carma::tests::test_tc_out_row_rvalue,
        "Test type caster"
    );
}
...
void bind_test_tc_out_col_rvalue(py::module &m) {
    m.def(
        "tc_out_col_rvalue",
        &carma::tests::test_tc_out_col_rvalue,
        "Test type caster"
    );
}
...
void bind_test_tc_out_cube_rvalue(py::module &m) {
    m.def(
        "tc_out_cube_rvalue",
        &carma::tests::test_tc_out_cube_rvalue,
        "Test type caster"
    );
}
